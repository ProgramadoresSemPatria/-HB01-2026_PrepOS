...
import { useLeetCodeProblems, useEvaluateSolution, type LeetCodeProblem } from '../../lib/api';
...
const ProblemCard = ({ 
  ...
  >
    <div className="flex justify-between items-start mb-4 gap-3">
      <h3 className="font-bold text-white leading-tight flex-1">{problem.title}</h3>
      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider whitespace-nowrap shrink-0 ${
...
export function LeetCodePage() {
  ...
  const gapsString = gaps.map(g => typeof g === 'string' ? g : g.skill).join(', ');

  const { data: problems, isLoading: isLoadingProblems } = useLeetCodeProblems(stack, seniority, gapsString);
  const { mutate: evaluateSolution, data: evaluation, isPending: isEvaluating } = useEvaluateSolution();
  ...
  return (
    <div className="w-full max-w-7xl mx-auto h-full flex flex-col pb-6">
      ...
      <header className="mb-8 border-b border-gray-800 pb-6">
        ...
        <p className="text-[#9a9a9a] mt-2">Resolva desafios focados nas tecnologias que você precisa aprimorar.</p>
      </header>
        ...
        <aside className="w-full md:w-1/3 flex flex-col gap-4">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
            ...
            {!isLoadingProblems && problems?.length === 0 && (
              <p className="text-gray-500 text-sm text-center py-6">Nenhum desafio encontrado para este perfil.</p>
            )}
          </div>
        </aside>
              ...
              <div className="flex-1 bg-[#202020] border border-gray-800 rounded-2xl p-6 flex flex-col shadow-lg">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-sm text-[#9a9a9a] mt-1">Escreva sua solução em TypeScript abaixo:</p>
                  </div>
                  <button 
              ...
              {evaluation && (
                <div className="bg-[#202020] border border-gray-800 rounded-2xl p-6 shadow-lg animate-fade-in-up">
                  <div className="flex items-center justify-between mb-6 border-b border-gray-800 pb-4">
                    <h3 className="text-lg font-bold text-white">Resultado da Avaliação</h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div className="bg-[#171717] p-4 rounded-xl border border-gray-800">
                      <p className="text-xs text-[#9a9a9a] uppercase tracking-wider mb-1">Complexidade de Tempo</p>
                      <p className="font-mono text-[#3ecf8e] font-medium">{evaluation.time_complexity}</p>
                    </div>
                    <div className="bg-[#171717] p-4 rounded-xl border border-gray-800">
                      <p className="text-xs text-[#9a9a9a] uppercase tracking-wider mb-1">Complexidade de Espaço</p>
                      <p className="font-mono text-[#3ecf8e] font-medium">{evaluation.space_complexity}</p>
                    </div>
                  </div>
                    ...
                    {evaluation.improvements.length > 0 && (
                      <div>
                        ...
                        <ul className="list-disc pl-5 text-sm text-gray-300 space-y-2">
                          {evaluation.improvements.map((imp, idx) => <li key={idx}>{imp}</li>)}
                        </ul>
                      </div>
                    )}
